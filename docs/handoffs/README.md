...
| 06 | [Web GUI, analytics and clinical presentation](06-web-gui-analytics-and-clinical-presentation.pdf) | **Supersedes 05 where they differ.** Part I reprints handoff 05; Part II is the coding annex — an 80-screen route atlas, the `ClinicianPatientProjection` contract, queue ordering, 20 page examples, 22 chart screens, the projection/API/component architecture, and a six-wave build order | **◐ In progress** — [`../site/gui-decisions.md`](../site/gui-decisions.md). **80 of 80 screens; 22 of 22 charts.** The screen atlas is complete, and §26's "keep accountability through transfer" is now built rather than refused — `/clinician/handoffs` models a transfer as proposed-then-accepted, with accountability staying put until somebody accepts. Three screens still refuse honestly and should: member and clinician messaging (no store, no recipient, no delivery path — a composer without a monitored reader is worse than no screen) and the schedule (no scheduling model). Remaining: the non-screen items in §§30–31 |
| 07 | [Demo login, synthetic population and planning engine](07-demo-login-synthetic-population-and-planning-engine.pdf) | Role-selectable demo access for six roles, 240 fabricated longitudinal patients across the four U.S. Census regions, a versioned metric dictionary and cohort registry, seven deterministic planning rules with an eight-state review machine, fairness controls, a model-registry shell, five screens, twelve APIs and ten release gates | **✅ Waves 0–8 done.** Demo identity, seed manifest, deterministic generator, role projections, metrics, planning rules; **Wave 7 governance** (fairness controls over the real 240, the p43 audit screen, the shadow-only model-registry shell) landed 2026-09-09 — this line said "open" for a day after it shipped, which is what a status doc does when nobody re-reads it. **Wave 8 closed 2026-09-10:** the clock, the typed-reason reset and p56's presenter scripts (as handoff 09 Package 4's guided walkthroughs), plus data-scenario injection, projection validation with expected hashes in the seed manifest, the QA export through the governed export path, and the nightly reset (off unless armed, UTC, skips a held walkthrough rather than interrupting it). All six of p9's controls are built, and `/admin/demo` no longer carries a not-built list. [`07-PLAN.md`](07-PLAN.md) |
