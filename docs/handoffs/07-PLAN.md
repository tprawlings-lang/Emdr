...
**Wave 8 is complete — the last four closed 2026-09-10.** Re-checked 2026-09-09 before that: Handoff 09 Packages 4 and 5 built
...
| Scenario injection | **Built.** `src/lib/demo/data-scenario.ts` — approved, versioned event bundles declared in code and reviewed like code. There is no upload and no free-text event: a console that can write arbitrary events into a clinical spine can fabricate a safety history. A bundle appends to the spine and nothing else, so it cannot invent a state the replay would not produce; the cohort is a filter over declared manifest columns rather than a search over outcomes; applying the same version twice is refused; and reversal is reset, with the applications table in `DEMO_DATA_TABLES` so that is a fact rather than a sentence |
| Projection validation | **Built.** Per-table expected hashes in `src/lib/demo/projection-hashes.generated.ts`, regenerated by `npm run gen:projection-hashes` (which resets twice and refuses to write if the two disagree). Per table rather than one number, because p9 says "fail the page if any ROLE'S VIEW differs" and a single hash cannot name which. It answers the question `verifyProjections` cannot: both can pass while the dataset is not the published one. An unrecorded hash fails rather than passes, and p29's gate now blocks on it — the sentence was half-implemented in the direction that does not announce itself |
| QA export | **Built.** Released through `createExport` rather than around it: stated purpose, content hash, signature, audit event before the file exists. Its own reader at `/api/demo/qa-report/[id]` under `requireDemoAdmin`, because the aggregate route resolves org and payer scope and excludes the platform tenant — the alternative was giving a demo administrator an intelligence binding to solve a routing problem. The one control here that does **not** refuse on a failing environment: it exists to describe a broken one to somebody who is not in the room |
| Nightly reset | **Built, and off by default.** `EMDR_DEMO_NIGHTLY_RESET=1` arms it and nothing else does — it is the only scheduled job that deletes member data and it runs at an hour chosen so nobody is watching. p57's second question is answered **UTC** (`EMDR_DEMO_RESET_HOUR_UTC`, default 09:00), because the environment has reviewers in several places and a timezone of its own in none. A held walkthrough **skips the night rather than being interrupted** — §7.3 allows an interruption only when an operator deliberately makes one, and a timer cannot be deliberate; a stale lock still counts. Every run records its duration against p29's 120s ceiling, reported and never enforced |
