      ...
      <header className="sticky top-0 z-10 -mx-6 border-b border-ground/10 bg-ivory/95 px-6 py-4 backdrop-blur">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 gap-2">
            <Link
              href={`/clinician/clinical/${person.id}`}
              ...
              <ul className="mt-3 space-y-2">
                {summary.claims.map((claim, n) => (
                  <li key={n} className="text-sm text-ground/90">
                    ...
                    <Link
                      href={`/clinician/clinical/${person.id}#timeline`}
        ...
        <aside className="space-y-4">
          <div className="rounded-3xl border border-ground/10 bg-linen p-4">
            ...
            <div className="mt-2 flex flex-col gap-2">
              <Link href={`/clinician/clinical/${person.id}`} className="text-sm text-state-info underline">
                Review summary and record
              </Link>
              <Link href={`/clinician/clinical/${person.id}#timeline`} className="text-sm text-state-info underline">
                Unified timeline
              </Link>
              <Link href={`/clinician/audit?person=${person.id}`} className="text-sm text-state-info underline">
                Audit history
              </Link>
            </div>
